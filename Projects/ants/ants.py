...
class ThrowerAnt(Ant):
    ...
    min_range = 0
    max_range = float('inf')
    def nearest_bee(self, hive, min_range = min_range, max_range = max_range):
        ...
        possible_bee_place = self.place 
        count = 0
        while count < self.min_range:
            possible_bee_place = possible_bee_place.entrance
            count = count + 1
        while count < self.max_range and possible_bee_place.bees == [] and possible_bee_place.entrance != hive:
            possible_bee_place = possible_bee_place.entrance
            count = count + 1 
        return random_or_none(possible_bee_place.bees)
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= amount:
            place_bees_copy = list(self.place.bees)
            for bee_i in place_bees_copy:
                bee_i.reduce_armor(self.damage)
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 7
        flypast_bees_copy = list(self.place.bees)
        for flypast_bee in flypast_bees_copy:
            flypast_bee.reduce_armor(self.damage)
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        ...
        if self.contained_ant:
            self.contained_ant.action(colony)
        bees_damaged = list(self.place.bees)
        for bee in bees_damaged:
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        QueenAnt.instances = QueenAnt.instances + 1
        # END Problem 13
    ...
    def action(self, colony):
        ...
        if self.is_imposter:
            ...
            while next_place:
                if next_place.ant:
                    if next_place.ant.is_container and next_place.ant.contained_ant and not next_place.ant.contained_ant.is_doubled:
                        next_place.ant.contained_ant.damage = next_place.ant.contained_ant.damage * 2
                        next_place.ant.contained_ant.is_doubled = True
                    if not next_place.ant.is_doubled:
                        next_place.ant.damage = next_place.ant.damage * 2
